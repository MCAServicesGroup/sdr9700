...
inline MemoryType radioMemoryFromRecord(const MemoryRecord& memory, quint16 group, quint16 channel)
{
    ...
    radioMemory.scan = static_cast<quint8>(std::clamp(memory.scan, 0, 3));
    ...
    radioMemory.filter = static_cast<quint8>(std::clamp(memory.filter, 1, 3));
    radioMemory.datamode = static_cast<quint8>(std::clamp(memory.dataMode, 0, 1));
    radioMemory.duplex = radioDuplexFromRecord(memory.duplexMode);
    radioMemory.tonemode = static_cast<quint8>(memory.toneMode);
    radioMemory.dsql = static_cast<quint8>(std::clamp(memory.dsql, 0, 2));
    radioMemory.dtcsp = static_cast<quint8>(std::clamp(memory.dtcsPolarity, 0, 3));
    radioMemory.dtcspB = static_cast<quint8>(std::clamp(memory.dtcsPolarityB, 0, 3));
    radioMemory.dvsql = static_cast<quint8>(std::clamp(memory.dvSql, 0, 99));
